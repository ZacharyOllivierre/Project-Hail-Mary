...
namespace engine::physics
{
    ...
    namespace
    {
        void add_debug_rect(
            ...
            engine::core::DebugDrawCategory category) noexcept
        {
            ...
            for (int substep = 0; substep < substep_count; ++substep)
            {
                const float allowed_x = resolve_horizontal_move(
                    *_collision_world,
                    entry.collider->collision_rect(),
                    substep_move.x);
                collided_x = collided_x || std::fabs(allowed_x - substep_move.x) > engine::core::Vector2::k_epsilon;
                ...
                const float allowed_y = resolve_vertical_move(
                    *_collision_world,
                    entry.collider->collision_rect(),
                    substep_move.y);
                collided_y = collided_y || std::fabs(allowed_y - substep_move.y) > engine::core::Vector2::k_epsilon;
