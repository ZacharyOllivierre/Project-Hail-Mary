...
namespace engine::scene
{
	...
	namespace
	{
		...
		[[nodiscard]] bool scene_object_input_event_less(
			const engine::core::SceneObject *lhs, const engine::core::SceneObject *rhs) noexcept
		{
			if (lhs == rhs)
				return false;
			...
			if (lhs_game && rhs_game)
			{
				return std::make_tuple(
						   ...
						   lhs_game->order_in_layer()) > std::make_tuple(rhs_game->depth_layer(),
	...
	void Scene::on_input(const engine::input::InputSnapshot &input, const std::vector<engine::input::InputEvent> &events)
	{
		...
		for (const engine::input::InputEvent &input_event : events)
		{
			if (input_event.action != engine::input::InputAction::Tab || input_event.type != engine::input::InputEventType::Pressed)
			{
				continue;
			}
	...
	void Scene::remove_destroyed_objects()
	{
		...
		for (auto &layer : _object_layers)
		{
			// Crash fix (whole for loop)
			for (const std::unique_ptr<engine::core::GameObject> &object : layer)
			{
				if (!object || !object->is_destroyed())
					continue;

				_physics_manager.unregister_body(object.get());
				engine::physics::CollisionManager::instance()->unregister_boxes(object.get());
			}
